...
def animate(i):
    ...
    # Fazendo a soma acumulada dos valores
    y1 = data.loc[data['group'] == 'control', 'click'].cumsum(
    ) / data.loc[data['group'] == 'control', 'visit'].cumsum()

    x2 = np.arange(len(data.loc[data['group'] == 'treatment']))
    # Fazendo a soma acumulada dos valores
    y2 = data.loc[data['group'] == 'treatment', 'click'].cumsum(
    ) / data.loc[data['group'] == 'treatment', 'visit'].cumsum()
